...
begin
	call raise_notice(format('set_order_payment | start with:'));	
	call raise_notice(format('set_order_payment | 	value  : ' || value_));	
	...
	call raise_notice(format('set_order_payment | 	name_         : ' || name_));	
	call raise_notice(format('set_order_payment | 	account_before: ' || account_before));	
	call raise_notice(format('set_order_payment | 	account_      : ' || account_));	
	...
	select c.account into result_account from public.customer c
	 	where c.id = customer_id_;
	call raise_notice('set_order_payment | result_account: ' || result_account);
	call raise_notice('set_order_payment | result_error  : ' || coalesce(result_error, ''));
