...
class EntryPayCustomer implements SchemaEntryAbstract {
  ...
  //
  @override
  EntryPayCustomer.from(Map<String, dynamic> row):
    _entry = SchemaEntry.from(row, def: _initial),
    _isEmpty = false;
  //
  //
  EntryPayCustomer.empty(): 
  ...
  ///
  /// CUSTOMER
  static Sql updateSqlBuilder(Sql sql, EntryPayCustomer entry) {
    if (entry.isEmpty) {
  ...
  ///
  ///
  static Sql insertSqlBuilder(Sql sql, EntryPayCustomer entry) {
    if (entry.isEmpty) {
      return Sql(sql: "select ;");
    } else {
  ...
  static Sql deleteSqlBuilder(Sql sql, EntryPayCustomer entry) {
    if (entry.isEmpty) {
      return Sql(sql: "select ;");
    } else {
      return Sql(sql: """UPDATE customer SET (
